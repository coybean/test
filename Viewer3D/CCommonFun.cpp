...
CCommonFun* CCommonFun::m_pSelf=nullptr;
...
CCommonFun *CCommonFun::getInstance()
{
    if(m_pSelf==nullptr)
    {
        m_pSelf = new CCommonFun();
    }
    return m_pSelf;
}

void CCommonFun::Destory()
{
    if(m_pSelf!=nullptr)
    {
        delete m_pSelf;
...
cv::Mat CCommonFun::QImage2Mat(QImage image)
{
    cv::Mat mat;
    switch(image.format())
    {
...
QImage CCommonFun::Mat2QImage(cv::Mat mat)
{
    QImage image;
    switch(mat.type())
    {
